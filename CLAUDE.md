...
**Media-admin**: Medielista har en egen rutenett-visning (`admin/components/media/media-grid.tsx`, montert som `beforeListTable`) som skjuler Payloads tabell med css i stedet for å erstatte listevisningen — da beholdes søk, filtre, mapper, paginering og masse-sletting. Alt-tekst fylles automatisk av en vision-modell ved opplasting (`lib/ai/alt-text.ts`). «Brukt på»-panelet (`lib/media-usage.ts`) utleder hvor et bilde står ved å følge fremmednøklene til `media(id)` opp til dokumentet, så nye blokker dekkes automatisk. Duplikatsøket (`lib/media-hash.ts` + `lib/media-duplicates.ts`) bruker sha256 + dHash beregnet ved opplasting; eksisterende bilder må få avtrykk via `scripts/backfill-media-hashes.ts`.
