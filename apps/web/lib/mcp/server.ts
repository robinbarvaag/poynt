...
export const mcpHandler = createMcpHandler(
  () => {
    const server = new McpServer(
      { name: "poynt-cms", version: "1.0.0" },
      {
        ...
        // Verktøylista er statisk (bygget av koden, ikke av data), så det finnes
        // ingen endringer å varsle om. Uten `listChanged` har klienten ingen
        // grunn til å åpne en `subscriptions/listen`-strøm — og på Vercel ville
        // en slik strøm holdt funksjonen i live til maxDuration og gitt «Task
        // timed out after 60 seconds» i loggen. Ruta avviser den uansett.
      ...
      "add_blocks_to_page_draft",
      {
        ...
        inputSchema: z.object({
          ...
          position: z
            ...
            .default("end")
            .describe(
              "Hvor blokkene settes inn. «after» krever afterBlockName."
            ),
          afterBlockName: z
            .string()
            .optional()
      ...
      async ({ idOrSlug }) => {
        ...
        return text({
          ...
          description: doc.description
            ? lexicalToMarkdown(doc.description)
            : "",
          ...
          featuredImageId: doc.featuredImage
            ? mediaId(doc.featuredImage)
            : null,
    ...
    server.registerTool(
      ...
      async ({ url, alt, filename }) => {
        ...
        let res: Response;
        try {
          // Uten tidsavbrudd kan en treg vert holde hele funksjonen til Vercels
          // maxDuration og gi «Task timed out» i stedet for en lesbar feil.
          res = await fetch(url, {
            cache: "no-store",
            redirect: "follow",
          ...
          return fail(
            timedOut
              ? "Verten svarte ikke innen 20 sekunder. Sjekk at lenken går rett til bildefila."
              : `Kunne ikke hente bildet: ${err instanceof Error ? err.message : String(err)}`
          );
        }
        if (!res.ok)
          return fail(`Kunne ikke hente bildet (HTTP ${res.status}).`);
  ...
  {
    // Uten denne svelges protokollfeil i stillhet — de er eneste spor når en
    // klient oppfører seg rart.
