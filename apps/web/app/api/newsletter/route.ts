...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { HONEYPOT_FIELD, describeSubmission, isHoneypotFilled } =
      await import("@/lib/spam-heuristics");
    if (isHoneypotFilled(body[HONEYPOT_FIELD])) {
      console.warn(
        `[spam] avviste nyhetsbrev-påmelding: honningkrukke fylt — ${describeSubmission(
          {
            epost: body.email,
            sti: body.path,
